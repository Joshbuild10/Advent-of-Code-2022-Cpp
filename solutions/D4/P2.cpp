...
public:
    ...
    bool overlappingPairs()
    {
        for (int i = assignment[0].start; i <= assignment[0].stop; i++)
        {
            if (i >= assignment[1].start && i <= assignment[1].stop)
            {
                return 1;
            }
        }
...
int redundancyCount(std::vector<assignmentPair> assignments)
{
    ...
    {
        sum += assignment.overlappingPairs();
    }
    return sum;
}
...
int main()
{
    ...
    read(data);
    std::cout << "The number of overlapping pairs are: " << redundancyCount(data) << "\n";
